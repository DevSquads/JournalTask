...
async function approveArticle(req, res, next) {
  try {
    ...
    let checkApprove = await Article.findOne({ _id: articleId });
    if (checkApprove && checkApprove.approve) return res.status(400).json({ message: 'Already Approved' });

    let approvedArticle = await Article.updateOne({ _id: articleId }, { approve: true }, { new: true });
